...
export default function CheckBoxComp({ setVal }) {
  const [isChecked, setIsChecked] = useState(false); // Initial state (optional)

  const handleCheckboxChange = (event) => {
    setIsChecked(event.target.checked);
    setVal("state", event.target.checked);
  };

  return (
    <div className="flex flex-row items-center w-full p-2 text-gray-500 space-y-4">
