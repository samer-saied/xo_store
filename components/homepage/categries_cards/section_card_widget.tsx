...
interface SectionCardWidgetProps {
  image: string | null; // Assuming URL or React component
  text: string; // Assuming plain text
  firstColor: string; // Assuming hexadecimal or CSS color
  secandColor: string; // Assuming hexadecimal or CSS color
}
...
export default function SectionCardWidget({
  ...
  firstColor,
  secandColor,
}: SectionCardWidgetProps) {
  return (
    <div
      style={{
        backgroundImage: `linear-gradient(to bottom,${firstColor}, ${secandColor})`,
      }}
    ...
    >
      <div className="group">
        ...
        <div className="mx-auto  md:h-64 h-40 group-hover:opacity-60 transition delay-150 duration-300 ease-in-out">
          <div className="flex flex-col justify-center items-center h-full">
            ...
            <div className="h-2/4 w-full flex flex-row justify-center">
              <img className="p-3 h-full" src={image? image :"/vercel.svg"} alt="My Happy SVG" />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-row justify-center items-center">
          <div className="transition duration-300 delay-150 ease-in-out opacity-0 group-hover:opacity-100 w-4/5  absolute top-2/4 md:h-20 h-12 px-2 py-4 bg-white bg-opacity-70 rounded-2xl flex flex-col justify-center items-center gap-2 ">
            <div
