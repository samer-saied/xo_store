...
const BannerCardWidget = ({ id, title, descrption, image }: Banner) => {
  return (
    <div
      key={id}
      className="w-full md:py-10 my-5 flex flex-row justify-center items-center bg-white md:min-h-80 h-auto  mb-10"
    >
      {/* //GRAY BACKGROUND AND TEXTS AND IMAGE */}
      {/*  GRAY BACKGROUND */}
      <div className="md:relative bg-gray-100 md:w-9/12 w-11/12 h-full rounded-3xl pt-3 px-5 md:py-8 drop-shadow-md">
        <div className=" w-8/12">
          <div className="text-right text-slate-500 text-xl md:text-2xl font-bold md:py-8 py-5">
            {title}
          </div>
           {/*  TEXTS */}
           <div className=" md:w-1/2 h-full flex flex-col items-start pb-5">
              <div className="pb-3 text-right text-neutral-400 md:text-md text-sm">
                {descrption.trim()}
              </div>
              <button
        ...
        {/* //IMAGE BANNER */}
        <div className="absolute md:-left-16 -left-1 bottom-0 md:top-0  w-5/12  ">
